...
import torchaudio
import settings
from mongo_utils import get_mongo_collection, clear_mongo_collections
import threading
import datetime
...
# def auto_reserve_vcons(collection, max_total_size_gb):
#     """
#     Reserve a list of vCons for processing.
#     Finds vCons with status 'pending' and processing_start_time == None,
#     updates their processing_start_time to now, and returns their filenames.
#     """
#     max_total_size_bytes = max_total_size_gb * (1024**3)
    
#     now = datetime.datetime.now(datetime.timezone.utc)
#     # Find vCons to reserve
#     vcons = list(collection.find({
#         "being_processed_by": None
#     }, {"_id": 1, "filename": 1}).limit(1))
#     vcon_ids = [vcon["_id"] for vcon in vcons]
#     if vcon_ids:
#         # Update processing_start_time for all reserved vCons
#         collection.update_many(
#             {"_id": {"$in": vcon_ids}},
#             {"$set": {"processing_start_time": now}}
#         )
#     # Return the list of filenames (vCon URLs)
#     return [vcon["filename"] for vcon in vcons if "filename" in vcon]

# We have a thread that finds unprocessed vcons and reserves them,
# Additionally it downloads them. 
# Then, in our main thread, we will load the wavs and process them, deleting them as we go. 
...
#     start_auto_reserve_vcons_thread() # go online and automatically fill up wav_cache_to_process when it goes beneath a certain size
#     while True:
#         move_vcons_to_processing() # move all currently downloaded vcons into wav_cache_to_id
#         run_language_id() # run language id on all vcons in wav_cache_to_id
#         move_english_vcons() # Move all english vcons into wav_cache_en
#         move_non_english_vcons() # Move all non-english vcons into wav_cache_non_en
#         run_transcription_on_english_maybe() # run transcription on all english vcons, if the buffer is full.
#         run_transcription_on_non_en_maybe() #
#         time.sleep(1)

    # 

    # start a thread that autoreserves vcons and downloads the wavs

    # try:
    #     # First, reserve a list of vcons to process.
    #     # Now, we have a list of vcons to process.
    #     # We will process the entirety of the vcon. Loading a file twice seems unnecessary.
    #     # We will process the vcons in batches.
    #     # We will process the vcons in parallel.
    #     # We will process the vcons in a loop.
    #     # We will process the vcons in a loop.
