...
from vcon_utils import batch_to_audio_data, set_languages, set_transcript
...
def load_whisper_tiny():
    ...
    move_to_gpu_maybe(model)
    move_to_gpu_maybe(processor)
...
def transcribe_many(vcons_batched, model):
    ...
    for vcon_batch in vcons_batched:
        ...
        for vcon, transcription in zip(vcon_batch, all_transcriptions):
            set_transcript(vcon, transcription)
            vcons.append(vcon)
        gc.collect()
    return vcons
